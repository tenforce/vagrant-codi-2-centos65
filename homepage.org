...
There are two CODI development environments for available, these
being:

- UnifiedViews 2.3.0 environment :: which is defined by the
     *install_uv_components.sh* script. The following tools are
...
bother cases the scripts function in a three stage approach. These
stages being defined:

- DOWNLOAD_BUILD :: which will likely require a connection to the
     internet to allow downloading of missing components to be
     recovered.
